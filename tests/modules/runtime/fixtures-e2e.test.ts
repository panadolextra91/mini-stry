...
describe("End-to-End Fixtures: validateAndEvaluate pipeline", () => {
  ...
  describe("Fixture A: Small numeric leave-days policy", () => {
    const policy = {
      rules: [
        { id: ruleId("A1"), when: { type: "compare" as const, field: "leave_days", op: "lte" as const, value: 2 }, decision: autoApprove() },
        { id: ruleId("A2"), when: { type: "compare" as const, field: "leave_days", op: "gt" as const, value: 14 }, decision: autoReject() }
      ],
    ...
    it.each([
      ...
      { leave_days: 5, expectedKind: "request-approval", expectedRule: null, expectedTrace: 2, expectedRole: "ROLE-MANAGER" }
    ])("leave_days=$leave_days -> $expectedKind", ({ leave_days, expectedKind, expectedRule, expectedTrace, expectedRole }) => {
  ...
  describe("Fixture B: String role-based policy", () => {
    const policy = {
      rules: [
        { id: ruleId("B1"), when: { type: "compare" as const, field: "requester_role", op: "eq" as const, value: "admin" }, decision: autoApprove() },
        { id: ruleId("B2"), when: { type: "compare" as const, field: "requester_role", op: "in" as const, value: ["intern", "contractor"] }, decision: autoReject() }
      ],
      defaultDecision: requestApproval(roleId("ROLE-DIRECTOR"))
    };

    it.each([
      { requester_role: "admin", expectedKind: "auto-approve", expectedRule: "B1", expectedTrace: 1 },
      { requester_role: "intern", expectedKind: "auto-reject", expectedRule: "B2", expectedTrace: 2 },
      { requester_role: "manager", expectedKind: "request-approval", expectedRule: null, expectedTrace: 2, expectedRole: "ROLE-DIRECTOR" }
    ])("role=$requester_role -> $expectedKind", ({ requester_role, expectedKind, expectedRule, expectedTrace, expectedRole }) => {
