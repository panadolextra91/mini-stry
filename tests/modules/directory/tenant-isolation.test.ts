...
describe("Tenant Isolation", () => {
  ...
  it("prevents Tenant B from seeing Tenant A's users", async () => {
    const roleA = await roleService.createRole(TENANT_A, { name: "admin" });
    const userA = await userService.createUser(TENANT_A, { email: "a@a.com", name: "A", roleId: roleA.id, managerId: null });
  ...
  it("prevents cross-tenant profile updates", async () => {
    ...
    await expect(userService.updateUserProfile(TENANT_B, userA.id, { name: "B" })).rejects.toThrow(UserNotFoundError);
  });
  ...
  it("prevents cross-tenant role assignment", async () => {
    ...
    await expect(userService.assignRole(TENANT_A, userA.id, roleB.id)).rejects.toThrow(RoleNotFoundError);
  });
});
...
describe("Adapter-Level Belt-and-Braces Tenant Isolation", () => {
  it("prevents cross-tenant modifications on ConvexRoleRepository", async () => {
    ...
    await expect(repo.rename(TENANT_A, roleId("role_1"), "x")).rejects.toThrow(/not found in tenant/);
  ...
  it("prevents cross-tenant modifications on ConvexUserRepository", async () => {
    ...
    await expect(repo.updateProfile(TENANT_A, userId("user_1"), { name: "x" })).rejects.toThrow(/not found in tenant/);
    await expect(repo.updateRole(TENANT_A, userId("user_1"), roleId("role_2"))).rejects.toThrow(/not found in tenant/);
    await expect(repo.updateManagerId(TENANT_A, userId("user_1"), userId("manager_1"))).rejects.toThrow(/not found in tenant/);
