...
describe("UserService", () => {
  ...
  describe("createUser", () => {
    it("creates user successfully", async () => {
      ...
      const user = await userService.createUser(TENANT_A, {
        email: "u@example.com",
        name: "U",
        roleId: role.id,
        managerId: null,
    ...
    it("rejects duplicate email", async () => {
      ...
      await expect(
        userService.createUser(TENANT_A, {
          email: "u@example.com",
          name: "U2",
          roleId: role.id,
          managerId: null,
    ...
    it("rejects invalid manager", async () => {
      ...
      await expect(
        userService.createUser(TENANT_A, {
          ...
          name: "U",
          roleId: role.id,
          managerId: userId("bad-id"),
        }),
      ).rejects.toThrow(ManagerNotFoundError);
    });
  });
  ...
  describe("updateUserProfile", () => {
    ...
    it("throws if not found", async () => {
      await expect(
        userService.updateUserProfile(TENANT_A, userId("bad-id"), { name: "N" }),
  ...
  describe("assignRole", () => {
    ...
    it("rejects cross-tenant role", async () => {
      ...
      await expect(userService.assignRole(TENANT_A, user.id, r2.id)).rejects.toThrow(
        RoleNotFoundError,
      );
    });
  });
  ...
  describe("deleteUser", () => {
    ...
    it("throws if missing", async () => {
      await expect(userService.deleteUser(TENANT_A, userId("bad-id"))).rejects.toThrow(
        UserNotFoundError,
      );
    });
  });
