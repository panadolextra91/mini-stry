...
- **Module Boundary Rule (D-08):** _"Cross-module imports are ALLOWED. Cross-module coupling is NOT."_ Cross-module imports MUST go through the barrel `index.ts`. Deep imports into `domain/`, `application/`, `adapters/` are forbidden.
...
} from "@/modules/directory/application/role-service.js";
...
describe("RoleService.createRole", () => {
  ...
  it("rejects duplicate name within the same tenant", async () => {
    ...
    await expect(service.createRole(tCtx, { name: "approver" })).rejects.toBeInstanceOf(
      RoleNameAlreadyExistsError,
    );
  });
