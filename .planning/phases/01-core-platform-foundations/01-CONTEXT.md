...
- **Module Boundary Rule (D-08):** *"Cross-module imports are ALLOWED. Cross-module coupling is NOT."*
...
- **D-19 — TenantContext (NEW first-class concept):**
  ...
  - **Conceptual parallel with EvaluationContext:**
    - `TenantContext`: *"Who owns this operation?"* (operational envelope — every service call)
    - `EvaluationContext`: *"What data is being evaluated?"* (runtime input — every policy evaluation)
