...
| `src/modules/approval/domain/state-machine.ts`                                | domain (pure fns)       | transform           | _new logic_ — pattern from RESEARCH Pattern 4; no direct analog                                                   | role-match (pure domain) |
...
export interface ApprovalChainRepositoryPort {
  ...
  ): Promise<ApprovalChain | null>; // reverse lookup + idempotency (Pitfall 5)
  updateStatus(
    ctx: TenantContext,
    id: ApprovalChainId,
    status: ChainStatus,
...
Copy verbatim: `Map<Id, Entity>` store, `idCounter` for deterministic IDs (`buildApprovalChainId(\`chain\_${this.idCounter++}\`)`), and the **tenant-scoping guard** in every read (analog lines 32-34):
...
export const fromApprovalChainId = (b: ApprovalChainId): Id<"approvalChains"> =>
  b as string as Id<"approvalChains">;
```
